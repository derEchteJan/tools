...
get_ip6()
{
    ip -6 address show dev $1 | grep "inet6" | head -2 | tail -1 | awk '/inet/ {print $2}' | awk -F'[\\\\/]' ' { print $1 }'
}
...
upload_ip6()
{
    ...
    echo "creating /tmp/server.conf"
    touch /tmp/server.conf
    echo "IP6=\"$2\"" > /tmp/server.conf
    echo "PORT=\"$3\"" >> /tmp/server.conf

    echo "checking ssh access to $ssh_host"
    ssh -o 'PasswordAuthentication=no' $ssh_host echo "ssh connection established"
    if [[ $? != 0 ]]; then echo "ssh: unable to connect to '$ssh_host'"; return 1; fi

    # copy /tmp/server.conf to remote container
    echo "uploading file /tmp/server.conf to $ssh_host:/tmp"
    scp /tmp/server.conf $ssh_host:/tmp/server.conf && echo "uploaded file"
    ...
    if [[ $container_pid != "" ]]
    then
        echo "copying uploaded file to container '$container_pid'"
        ssh $ssh_host "sudo docker cp /tmp/server.conf redirecter:/app/server.conf && rm /tmp/server.conf" && echo "file copied"
    else
        echo "no remote container running"
    fi
    echo "restarting container 'redirecter'"
    ssh $ssh_host "sudo docker restart redirecter" && echo "restarted"
}


echo "installing dependencies"
install_command ip iproute2
install_command socat socat
...
then
    ...
    do
        ...
        if [[ $PORT == "" ]]
        then
            echo "error: no forwareded port specified in server.conf"
            exit 1
        fi
...
then
    ...
    set_addr=""
    addr=$( get_ip6 $IF_NAME ) # MARK JAN: TODO this doesnt work as the device enp8s0 doesnt exist within the docker container
